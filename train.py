...
from models import ActionModel  # Simple CNN model for action classification
...
TRAIN_LABEL_DIR = "CV_Train/Labels"  # Assuming labels are in LabelMe XML format
...
def main():
    ...
    # --- 2. Define Model ---
    model = ActionModel(num_classes=NUM_CLASSES).to(DEVICE)
    ...
    # Note: Using a more compatible version of the scheduler without 'verbose' parameter
    ...
    final_model_path = os.path.join(
        CHECKPOINT_DIR, f"final_model_{timestamp}.pth")
    torch.save(model.state_dict(), final_model_path)
    print(f"Final model saved to {final_model_path}")

    # Also save to standard path for compatibility with existing code
    std_model_path = "action_detection_model.pth"
