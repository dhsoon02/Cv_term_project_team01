#!/usr/bin/env python
"""
LabelMe → YOLO txt 변환 + train/val 분할
usage:
  python tools/convert_labelme_to_yolo.py --src CV_Train --dst dataset_yolo --split_ratio 0.2
"""
import json, random, argparse, shutil, sys
from pathlib import Path
from PIL import Image

LABEL2ID = {"person":0}

def convert_one(jfile: Path, img_out: Path, lbl_out: Path):
    meta = json.loads(jfile.read_text())

    # ---------- 이미지 파일 위치 잡기 ----------
    img_rel = Path(meta["imagePath"])
    if img_rel.is_absolute():
        src_img = img_rel
    else:
        # 보통 'Images/xxxxx.png' 혹은 JSON과 같은 폴더
        cand1 = jfile.parent.parent / "Images" / img_rel.name
        cand2 = jfile.parent / img_rel
        cand3 = jfile.with_suffix(".png")
        for c in (cand1, cand2, cand3):
            if c.exists():
                src_img = c
                break
        else:
            print(f"[WARN] image not found: {img_rel} for {jfile}", file=sys.stderr)
            return  # skip this label

    dst_img = img_out / src_img.name
    shutil.copy2(src_img, dst_img)

    w, h = Image.open(src_img).size
    yolo_lines = []
    for sh in meta["shapes"]:
        (x1,y1), (x2,y2) = sh["points"]
        xc, yc = (x1+x2)/2/w, (y1+y2)/2/h
        bw, bh = (x2-x1)/w,  (y2-y1)/h
        #cls_id = LABEL2ID[sh["label"]]
        cls_id = 0  # 모든 라벨을 'person' 클래스로 통일
        yolo_lines.append(f"{cls_id} {xc:.6f} {yc:.6f} {bw:.6f} {bh:.6f}")
    (lbl_out / f"{dst_img.stem}.txt").write_text("\n".join(yolo_lines))

def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--src", default="dataset")
    ap.add_argument("--dst", default="dataset_yolo_only_person")
    ap.add_argument("--split_ratio", type=float, default=0.2)
    args = ap.parse_args()

    src = Path(args.src)
    dst = Path(args.dst)
    ratio = args.split_ratio

    # 출력 폴더 생성
    img_tr = dst/"images"/"train"; img_val = dst/"images"/"val"
    lbl_tr = dst/"labels"/"train"; lbl_val = dst/"labels"/"val"
    for p in [img_tr, img_val, lbl_tr, lbl_val]: p.mkdir(parents=True, exist_ok=True)

    # ---------- JSON 전부 수집 ----------
    json_files = sorted(src.rglob("*.json"))
    if not json_files:
        print(f"[ERROR] No JSON found under {src}. 경로 확인!", file=sys.stderr)
        sys.exit(1)

    random.seed(42); random.shuffle(json_files)
    val_set = set(json_files[: int(len(json_files)*ratio)])

    skipped = 0
    for jf in json_files:
        img_dir, lbl_dir = (img_val, lbl_val) if jf in val_set else (img_tr, lbl_tr)
        before = len(list(img_dir.glob("*.png")))
        convert_one(jf, img_dir, lbl_dir)
        after = len(list(img_dir.glob("*.png")))
        if after == before:
            skipped += 1

    print(f"✅ 변환 완료: train {len(list(img_tr.glob('*.png')))} "
          f"/ val {len(list(img_val.glob('*.png')))}  (skipped {skipped})")

if __name__ == "__main__":
    main()
