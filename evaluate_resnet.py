...
OUTPUT_DIR = "test_outputs_resnet"
...
RESNET_TYPE = 18  # Match the type used for training (18, 34, 50, 101, 152)
MODEL_NAME = f"resnet{RESNET_TYPE}_action"  # Used for loading models
DEFAULT_MODEL_PATH = f"{MODEL_NAME}.pth"  # Default model path

# Create output directory
os.makedirs(OUTPUT_DIR, exist_ok=True)

# Image transformation should match what was used in validation
...
def load_model(model_path, num_classes):
    ...
    model = ResNetActionModel(num_classes=num_classes,
                              pretrained=False, resnet_type=RESNET_TYPE)
...
def visualize_prediction(img, prediction, gt_label=None):
    ...
    title += f"\nInference Time: {prediction['time_ms']:.2f} ms"
    ax1.set_title(title)
    ax1.axis('off')
...
def save_results(predictions, metrics, output_dir):
    ...
    with open(results_file, 'w') as f:
        ...
        if metrics:
            ...
            if 'avg_inference_time' in metrics:
                ...
                f.write(
                    f"Max inference time: {metrics['max_inference_time']:.2f} ms\n")
                f.write(
                    f"Min inference time: {metrics['min_inference_time']:.2f} ms\n")
                f.write(
                    f"Images below 50ms threshold: {metrics['below_threshold_count']} ({metrics['below_threshold_percent']:.2f}%)\n")
    ...
    if 'inference_times' in metrics:
        ...
        plt.title(
            f'Inference Time Distribution (Avg: {metrics["avg_inference_time"]:.2f} ms)')
        plt.legend()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate ResNet action detection model")
    ...
    if args.best and os.path.exists(os.path.join(CHECKPOINT_DIR, f"{MODEL_NAME}_best.pth")):
    ...
    for img_name in test_images:
        ...
        inference_times.append(prediction['time_ms'])
        if prediction['time_ms'] < 50:
    ...
    metrics = {
        ...
        'below_threshold_percent': 100 * below_threshold_count / len(inference_times) if inference_times else 0
    }
    ...
    print(f"\nInference Speed:")
    print(f"- Average: {metrics['avg_inference_time']:.2f} ms")
    print(
        f"- Range: {metrics['min_inference_time']:.2f} - {metrics['max_inference_time']:.2f} ms")
    print(
        f"- Below 50ms threshold: {metrics['below_threshold_count']}/{len(inference_times)} images ({metrics['below_threshold_percent']:.2f}%)")
